...
@interface K3MASViewConstraint ()

@property (nonatomic, strong, readwrite) K3MASViewAttribute *secondViewAttribute;
@property (nonatomic, weak) UIView *installedView;
@property (nonatomic, weak) K3MASLayoutConstraint *layoutConstraint;
...
@implementation K3MASViewConstraint
...
- (id)copyWithZone:(NSZone __unused *)zone {
    K3MASViewConstraint *constraint = [[K3MASViewConstraint alloc] initWithFirstViewAttribute:self.firstViewAttribute];
...
- (void)setSecondViewAttribute:(id)secondViewAttribute {
    if ([secondViewAttribute isKindOfClass:NSValue.class]) {
        ...
        _secondViewAttribute = [[K3MASViewAttribute alloc] initWithView:secondViewAttribute layoutAttribute:self.firstViewAttribute.layoutAttribute];
    } else if ([secondViewAttribute isKindOfClass:K3MASViewAttribute.class]) {
        K3MASViewAttribute *attr = secondViewAttribute;
        if (attr.layoutAttribute == NSLayoutAttributeNotAnAttribute) {
            _secondViewAttribute = [[K3MASViewAttribute alloc] initWithView:attr.view item:attr.item layoutAttribute:self.firstViewAttribute.layoutAttribute];;
        } else {
    ...
    return ^id(id attribute, NSLayoutRelation relation) {
        if ([attribute isKindOfClass:NSArray.class]) {
            ...
            for (id attr in attribute) {
                K3MASViewConstraint *viewConstraint = [self copy];
...
- (void)install {
    ...
    K3MASLayoutConstraint *layoutConstraint
        = [K3MASLayoutConstraint constraintWithItem:firstLayoutItem
    ...
    K3MASLayoutConstraint *existingConstraint = nil;
...
- (K3MASLayoutConstraint *)layoutConstraintSimilarTo:(K3MASLayoutConstraint *)layoutConstraint {
    // check if any constraints are the same apart from the only mutable property constant

    // go through constraints in reverse as we do not want to match auto-resizing or interface builder constraints
    // and they are likely to be added first.
    for (NSLayoutConstraint *existingConstraint in self.installedView.constraints.reverseObjectEnumerator) {
        if (![existingConstraint isKindOfClass:K3MASLayoutConstraint.class]) continue;
